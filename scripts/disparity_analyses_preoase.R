...
# clade pollen grains (Pre-ordination ancestral state estimation version)
#
# 08-05-2023
#
# Rafael F. Barduzzi
...
## prune data
dat_pollen <- dat_pollen %>%
  filter(name_phylogeny %in% tree$tip.label)
...
dir.create("pre_ordination_ase", showWarnings = F)
...
## make tree full bifurcating and remove zero-branch lengths
tree_bifur <- multi2di(tree_pruned)
tree_bifur$edge.length[tree_bifur$edge.length == 0] <- 0.0000001

## estimate ancestral traits
cladistic_matrix <- estimate_ancestral_states(
  cladistic_matrix,
  tree_bifur,
...
## Plot phylo-morphospace (Supplementary Fig. S5) -----------------------------
...
data_vectors_genus <- as.data.frame(pcoa$vectors.cor[1:134, c (1, 2)])
...
phylomorphospace <-
  ...
  geom_point(data = data_vectors_genus,
             ...
             size = 3) + 
  geom_point(data = space_axis[135:267, ],
             aes(
               x = Axis.1,
               y = Axis.2),
             size = 0.7) +
...
data_vectors_eco <- pcoa$vectors.cor[1:134, c (1, 2)]
...
groups_ecological <- groups_ecological[sapply(groups_ecological, length) >= 3]
...
labels_mpd <- c(rep("Genera", 7), (rep("Clades", 2)), (rep("Biomes", 4)), 
...
labels <- c(rep("Genera", 7), (rep("Clades", 2)), (rep("Biomes", 4)), 
            (rep("Ecological lineages", 5)))


if (rarefaction == "min") { # rewrite if using rarefaction
  
  labels <- c(rep("Genera", 12), (rep("Clades", 3)), (rep("Biomes", 7)), 
...
statistic_W_sv <-
  c(
    ...
    unlist(ecological_sv_wilcox)[c(1:6)],
...
p_value_sv <-
  c(
    ...
    unlist(ecological_sv_wilcox)[c(7:12)],
...
saveWorkbook(wb, "pre_ordination_ase/mean_pairwise_distance.xlsx", overwrite = TRUE)
...
saveWorkbook(wb, "pre_ordination_ase/sum_of_variances.xlsx", overwrite = TRUE)
...
#### Ecological groups
E1 <- c(rep("Flooded grassland", bootstraps))
E2 <- c(rep("Savanna", bootstraps))
E3 <- c(rep("SDTF", bootstraps))
E4 <- c(rep("Tropical rainforest", bootstraps))
...
SV_Flooded_grassland <- data.frame(E1, sum_Flooded_grassland)
colnames(SV_Flooded_grassland) <- c("Group", "Sum_of_variances")
SV_Savanna <- data.frame(E2, sum_Savanna)
colnames(SV_Savanna) <- c("Group", "Sum_of_variances")
SV_SDTF <- data.frame(E3, sum_SDTF)
colnames(SV_SDTF) <- c("Group", "Sum_of_variances")
SV_Tropical_rainforest <- data.frame(E4, sum_Tropical_rainforest)
...
plot_ecogroups_sv <-
  ...
  geom_boxplot(alpha = 0.5, show.legend = F) + labs(x = "", y = "Sum of variances") +
  scale_fill_manual(values = as.character(paletteer_d(
    "ggthemes::excel_Office_2007_2010"))[1:4]
  ) +
  scale_colour_manual(values = as.character(paletteer_d(
    "ggthemes::excel_Office_2007_2010"))[1:4]
  ) +
...
# SV Boxplots - rarefied (Supplementary Fig. S6) ==============================
...
bhatt_pairwise_biomes <- summary_stats_sv[c(23:28), c(2,5)] %>%
  separate(Groups, into = c("Taxon1", "Taxon2"), sep = " : ")
...
bhatt_pairwise_ecolineages <- summary_stats_sv[c(29:38), c(2,5)] %>%
  separate(Groups, into = c("Taxon1", "Taxon2"), sep = " : ")
...
pdf("pre_ordination_ase/suppfig5_phylomorphospace_preoase.pdf")
...
pdf("pre_ordination_ase/morphospace-eco.pdf")
...
pdf("pre_ordination_ase/sum_of_variances.pdf", width = 10, height = 10)
...
if (rarefaction == "min") {
  ...
  pdf("pre_ordination_ase/suppfig6_sum_of_variances_rarefied.pdf", width = 10, height = 10)
...
pdf("pre_ordination_ase/corr_matrices.pdf", width = 9, height = 9)
